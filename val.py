...
def validate_unet(model, val_dataloader, criterion, device, dataset_weights):
    ...
    with torch.no_grad():  # No gradient calculation for validation
        for images, masks in tqdm(val_dataloader, desc="Validation"):
    ...
    avg_wiou = total_wiou / len(val_dataloader)
    total_iou_per_class = total_iou_per_class / len(val_dataloader)
    total_dice_per_class = total_dice_per_class / len(val_dataloader)

    print(f"✔️ Validation Loss: {avg_loss:.4f}, Dice Score: {avg_dice:.4f}, mIoU Score: {avg_miou:.4f}, wIoU Score: {avg_wiou:.4f}")

    return avg_loss, avg_dice, avg_miou, avg_wiou, total_iou_per_class, total_dice_per_class  # Return for logging (e.g., WandB)

def validate_pairunet(model, val_dataloader, criterion, device, dataset_weights):
    ...
    with torch.no_grad():  # No gradient calculation for validation
        for images_rgb, images_ir, masks in tqdm(val_dataloader, desc="Validation"):
